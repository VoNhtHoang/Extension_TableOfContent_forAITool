javascript:(function(){
    ...
    t.id="toc-panel",t.innerHTML='<div id="toc-header">Conversation TOC</div><ul id="toc-list"></ul>',document.body.appendChild(t);
    ...
    function l(){
        ...
        var d=(r||document).querySelectorAll("div[class^='relative group flex flex-col justify-center w-full']");
        ...
        for(var u=0;u<d.length;u++){
            var g=d[u],m=document.createElement("li"),h=g.querySelector(""),f=false;
            var z=Math.ceil(u/2), j=t[z];
                
                if(h&&h.textContent.indexOf("ChatGPT")>=0){
                f=true,m.textContent="Turn "+(u+1)+" (AI)"}else{m.textContent="Turn "+(u+1)+" (You): "+(j.textContent.slice(0,70))}
    ...
    function y(){
        var e=document.querySelector("main#main")||document.querySelector(".chat-container")||null;
        if(e!==r){
